...
const ALL_SENTENCES = [
  ...
  { emoji: "🏷️", text: "How much does this cost?" },
    ...
    >
      <div className="animate-slide-up w-full max-w-sm bg-[#f5f5f5] rounded-t-3xl shadow-xl flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between px-5 pt-6 pb-3 flex-shrink-0">
          <h2 className="text-xl font-bold text-[#7c5cbf]">Phrasebook</h2>
...
function TamilHomeInner() {
  ...
  useEffect(() => {
    ...
    router.replace("/tamil", { scroll: false });
  ...
  const handleShuffle = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sentences?lang=tamil&exclude=${encodeURIComponent(exclude)}`);
    ...
    <div className="min-h-screen bg-[#f0f0f0] flex justify-center">
      <div className="w-full max-w-sm bg-[#f0f0f0] pt-0 pb-8 overflow-hidden">
        ...
        <div className="relative w-full mb-4 -mx-4" style={{ width: "calc(100% + 2rem)" }}>
          <img src="/banner-tamil.png" alt="Learn to say anything in Tamil" className="w-full object-cover" />
        ...
        <div className="px-4">
          ...
          <button
            ...
            className="w-full bg-[#7c5cbf] hover:bg-[#6b4dab] text-white text-lg font-semibold py-4 rounded-full mb-6 transition-colors"
          >
          ...
          <div className="flex items-center justify-center gap-3 pb-2">
            <button
              ...
              className="flex items-center gap-2 bg-[#7c5cbf] hover:bg-[#6b4dab] disabled:opacity-60 active:scale-95 text-white font-semibold text-sm px-5 py-3 rounded-full transition-all shadow-sm"
            >
        ...
        >
          <div className="w-full max-w-sm bg-white rounded-t-3xl px-5 pt-5 pb-8 shadow-xl">
            ...
            <p className="text-sm text-gray-500 text-center mb-4">
              Type in the sentence you&apos;d like to learn in Tamil
            </p>
            <input
              ref={inputRef}
              type="text"
              ...
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 text-base outline-none focus:border-[#7c5cbf] shadow-sm mb-5"
            />
            <button
              disabled={!sentence.trim()}
              onClick={() => router.push(`/learn?sentence=${encodeURIComponent(sentence)}&emoji=📝&lang=tamil`)}
              className="w-full bg-[#7c5cbf] disabled:bg-[#b09cde] text-white text-lg font-semibold py-4 rounded-full transition-colors"
            >
              Continue
            </button>
          </div>
        ...
        >
          <div className="w-full max-w-sm bg-white rounded-3xl px-6 pt-6 pb-7 shadow-2xl relative">
            ...
            <div className="text-center mb-5">
              ...
              <p className="text-2xl font-bold text-[#7c5cbf] mt-1">&ldquo;{showLearnModal.text}&rdquo;</p>
              <p className="text-xl font-bold text-gray-800 mt-1">in Tamil</p>
            </div>
            ...
            <button
              onClick={() => router.push(`/learn?sentence=${encodeURIComponent(showLearnModal.text)}&emoji=${encodeURIComponent(showLearnModal.emoji)}&lang=tamil`)}
              className="w-full bg-[#7c5cbf] hover:bg-[#6b4dab] text-white text-lg font-semibold py-4 rounded-full transition-colors"
            >
              Start learning
            </button>
          </div>
